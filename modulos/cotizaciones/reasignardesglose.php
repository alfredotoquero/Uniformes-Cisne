...
<div style="width: 600px; height: 400px; padding-left: 20px; padding-right: 20px;">
    ...
    <form name="formDesglose" id="formDesglose" action="" method="post">
        <?
            ...
            <?
        ?>
        <input type="hidden" name="accion" value="reasignardesglose">
        ...
        <div class="row">
            ...
            foreach ($partidas["partidas"] as $partida) {
                ...
                ?>
                <div class="col-6">
                    <input type="checkbox" name="cotizacionproductos[]" id="producto<?= $partida["idtmp"] ?>" value="<?= $partida["idtmp"]; ?>" <? if($claseCotizaciones->tienePartidaEspecificacionTMP($_GET["idespecificacion"],$partida["idtmp"])){?> checked <?} ?>><?= " " . $partida["cantidad"] . " - " . $nombre . "<small>" . $detalle . "</small>"; ?>
                </div>
                <?
            }
            ?>
