...
$pedido = $clasePedidos->obtenerPedido($_GET)["pedido"];
...
$_POST["txtBusqueda"] = $_GET["txtBusqueda"];
$_POST["slcSurtido"] = $_GET["slcSurtido"];
$_POST["slcSucursal"] = $_GET["slcSucursal"];
$_POST["slcPendiente"] = $_GET["slcPendiente"];
$_POST["slcOrden"] = $_GET["slcOrden"];
$clientes = $claseProduccion->obtenerClientesProduccion2($_POST);

foreach($clientes["clientes"] as $cliente){

    $_POST["cliente"] = $cliente["cliente"];
    $_POST["idcliente"] = $cliente["idcliente"];
    // $idpedidos = $claseProduccion->obtenerIdpedidos($_POST)["idpedidos"];
    $idpedidos = explode(",",$cliente["pedidos"]);
    ...
    foreach($idpedidos as $idpedido){
        ...
        if($especificaciones["respuesta"]=="OK"){
            foreach($especificaciones["especificaciones"] as $especificacion) {
                ...
                $pdf->Cell(98,0,mb_convert_encoding('Pedido: '.$especificacion["idpedido"] . "-" . $cliente["cliente"], "latin1", "UTF-8"),0,0,'L');
                $pdf->Ln(4);
                ...
                // // si no tiene partidas
                // if ($textopartidas=="") {
                //     $textopartidas = "Sin partidas";
                // }
                ...
                // $pdf->MultiCell(98,5,mb_convert_encoding($textopartidas),0,'L');
                // $pdf->Ln(3);
                // $posy = $pdf->GetY();
